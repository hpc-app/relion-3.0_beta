#ifndef HIP_CUSTOM_ALLOCATOR_HIP_
#define HIP_CUSTOM_ALLOCATOR_HIP_
...
public:
	...
	class Alloc
	{
		friend class HipCustomAllocator;
...
//		_printState();
		...
		HipShortcuts::cpyDeviceToHost<BYTE>( a->guardPtr, guards, guardCount, stream);
...
public:
	...
	HipCustomAllocator(size_t size, size_t alignmentSize):
...
//		_printState();
		...
		if (cache)
		{
			...
			if (curAlloc == NULL)
			{
				...
				if (curAlloc == NULL)
				{
					printf("ERROR: HipCustomAllocator out of memory\n [requestedSpace:             %lu B]\n [largestContinuousFreeSpace: %lu B]\n [totalFreeSpace:             %lu B]\n",
		...
		hipStream_t stream = 0;
		HipShortcuts::memInit<BYTE>( newAlloc->guardPtr, GUARD_VALUE, size - requestedSize, stream); //TODO switch to specialized stream
		DEBUG_HANDLE_ERROR(hipStreamSynchronize(stream));
#endif
