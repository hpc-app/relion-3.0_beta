#ifndef HIP_HELPER_KERNELS_HPP_
...
template<typename T>
__global__ void hip_kernel_weights_exponent_coarse(
		T *g_pdf_orientation,
...
template<typename T>
__global__ void hip_kernel_exponentiate(
		T *g_array,
		T add,
		size_t size)
{
	int idx = hipThreadIdx_x + hipBlockIdx_x*BLOCK_SIZE;
	if(idx < size)
	{
		T a = g_array[idx] + add;
...
__global__ void hip_kernel_exponentiate_weights_fine(
...
__global__ void hip_kernel_initRND(unsigned long seed,
                                    hiprandState *States);

__global__ void hip_kernel_RNDnormalDitributionComplexWithPowerModulation2D( ACCCOMPLEX *Image,
                                                                            hiprandState *States,
                                                                            long int xdim,
                                                                            XFLOAT * spectra);

__global__ void hip_kernel_RNDnormalDitributionComplexWithPowerModulation3D( ACCCOMPLEX *Image,
                                                                            hiprandState *States,
                                                                            long int xdim,
                                                                            long int ydim,
                                                                            XFLOAT * spectra);

__global__ void hip_kernel_softMaskOutsideMap(	XFLOAT *vol,
												long int vol_size,
												long int xdim,
												long int ydim,
												long int zdim,
...
__global__ void hip_kernel_softMaskBackgroundValue(	XFLOAT *vol,
														long int vol_size,
														long int xdim,
														long int ydim,
														long int zdim,
...
__global__ void hip_kernel_cosineFilter(	XFLOAT *vol,
											long int vol_size,
											long int xdim,
											long int ydim,
											long int zdim,
...
template <typename T>
__global__ void hip_kernel_translate2D(	T * g_image_in,
											T * g_image_out,
											int image_size,
											int xdim,
											int ydim,
...
template <typename T>
__global__ void hip_kernel_translate3D(	T * g_image_in,
											T * g_image_out,
											int image_size,
											int xdim,
											int ydim,
...
//----------------------------------------------------------------------------
//__global__ void hip_kernel_selfTranslate2D(	XFLOAT * g_image_in,
//												XFLOAT * g_image_out,
//												int image_size,
//												int xdim,
//												int ydim, //not used
...
//												int dy);
//
//__global__ void hip_kernel_selfTranslate3D(	XFLOAT * g_image_in,
//												XFLOAT * g_image_out,
//												int image_size,
//												int xdim,
//												int ydim,
...
//												int dz);
//----------------------------------------------------------------------------
//__global__ void hip_kernel_powerClass2D(	ACCCOMPLEX * g_image,
//											XFLOAT * g_spectrum,
//											int image_size,
//											int spectrum_size,
//											int xdim,
...
//
//__global__ void hip_kernel_powerClass3D(	ACCCOMPLEX * g_image,
//											XFLOAT * g_spectrum,
//											int image_size,
//											int spectrum_size,
//											int xdim,
...
//----------------------------------------------------------------------------
__global__ void hip_kernel_centerFFT_2D(XFLOAT *img_in,
										 int image_size,
										 int xdim,
										 int ydim,
										 int xshift,
...
__global__ void hip_kernel_centerFFT_3D(XFLOAT *img_in,
										 int image_size,
										 int xdim,
										 int ydim,
										 int zdim,
...
__global__ void hip_kernel_multi( T *A,
								   ...
								   T S,
		  	  	  	  	  	  	   int image_size)
{
	int pixel = hipThreadIdx_x + hipBlockIdx_x*BLOCK_SIZE;
	if(pixel<image_size)
		OUT[pixel] = A[pixel]*S;
}
 ...
 *  A[i] = A[i]*A[i]
 */
__global__ void hip_kernel_square(
		XFLOAT *A,
		int image_size);
...
__global__ void hip_kernel_cast(
		T1 *IN,
		T2 *OUT,
		int size)
{
...
__global__ void hip_kernel_frequencyPass(
										ACCCOMPLEX *A,
										long int ori_size,
										size_t Xdim,
										size_t Ydim,
										...
										int image_size)
{
	int texel = hipThreadIdx_x + hipBlockIdx_x*BLOCK_SIZE;
...
template<bool DATA3D>
__global__ void hip_kernel_powerClass(		ACCCOMPLEX * g_image,
											XFLOAT * g_spectrum,
											int image_size,
											int spectrum_size,
											int xdim,
											...
											XFLOAT * g_highres_Xi2)
{
	...
	if(voxel<image_size)
	{
		...
		if((ires>0.f) && (ires<spectrum_size) && coords_in_range)
		{
			...
			hip_atomic_add(&g_spectrum[ires], normFaux);
			if(ires>=res_limit)
	...
	if(tid==0)
		hip_atomic_add(&g_highres_Xi2[0], s_highres_Xi2[0]);
}

template<bool invert>
__global__ void hip_kernel_make_eulers_2D(
		XFLOAT *alphas,
		XFLOAT *eulers,
		unsigned orientation_num);

template<bool invert,bool doL, bool doR>
__global__ void hip_kernel_make_eulers_3D(
		XFLOAT *alphas,
		XFLOAT *betas,
		XFLOAT *gammas,
		XFLOAT *eulers,
...
template< typename T>
__global__ void hip_kernel_init_complex_value(
		T *data,
		XFLOAT value,
		size_t size)
{
	size_t idx = hipBlockIdx_x * INIT_VALUE_BLOCK_SIZE + hipThreadIdx_x;
	if (idx < size)
	{
		data[idx].x = value;
		data[idx].y = value;
...
template< typename T>
__global__ void hip_kernel_init_value(
		T *data,
		T value,
		size_t size)
{
...
__global__ void hip_kernel_allweights_to_mweights(
...
template< typename T>
__global__ void hip_kernel_array_over_threshold(
		T *data,
		bool *passed,
		T threshold,
		size_t size)
{
	size_t idx = hipBlockIdx_x * OVER_THRESHOLD_BLOCK_SIZE + hipThreadIdx_x;
	if (idx < size)
	{
...
template< typename T>
__global__ void hip_kernel_find_threshold_idx_in_cumulative(
		T *data,
		T threshold,
		size_t size_m1, //data size minus 1
		size_t *idx)
{
	size_t i = hipBlockIdx_x * FIND_IN_CUMULATIVE_BLOCK_SIZE + hipThreadIdx_x;
...
{
	unsigned n = hipThreadIdx_x + WINDOW_FT_BLOCK_SIZE * hipBlockIdx_x;
	long int image_offset = oX*oY*oZ*hipBlockIdx_y;
	if (n >= max_idx) return;
...
{
	size_t n = hipThreadIdx_x + WINDOW_FT_BLOCK_SIZE * hipBlockIdx_x;
