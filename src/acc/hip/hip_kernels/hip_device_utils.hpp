...
__device__ inline double hip_atomic_add(double* address, double val)
{
...
__global__ void hip_kernel_init_complex_value(
        ...
        int block_size)
{
	size_t idx = hipBlockIdx_x * block_size + hipThreadIdx_x;
	if (idx < size)
	{
		data[idx].x = value;
		data[idx].y = value;
...
{
	unsigned n = hipThreadIdx_x + block_size * hipBlockIdx_x;
