...
AutoPickerHip::AutoPickerHip(AutoPicker *basePicker, int dev_id, const char * timing_fnm) :
	node(NULL),
	basePckr(basePicker),
	allocator(new HipCustomAllocator(0, 1)),
	micTransformer(0, allocator),
	hipTransformer1(0, allocator),
#ifdef TIMING_FILES
	timer(timing_fnm),
#endif
	hipTransformer2(0, allocator)
...
AutoPickerHip::AutoPickerHip(AutoPickerMpi *basePicker, int dev_id, const char * timing_fnm) :
	basePckr(basePicker),
	allocator(new HipCustomAllocator(0, 1)),
	micTransformer(0, allocator),
	hipTransformer1(0, allocator),
...
{
	hipTransformer2.setSize(workSize,workSize,1);
	...
	hipLaunchKernel(hip_kernel_convol_B,Bsize,BLOCK_SIZE,   ~d_Fmic,
												  ~d_Fmsk,
												  ~d_Fcov,
												  d_Fmic.getSize());
	...
	hipLaunchKernel(hip_kernel_multi<XFLOAT>,Bsize,BLOCK_SIZE,
	...
	runCenterFFT(hipTransformer2.reals,
				 (int)hipTransformer2.xSize,
				 (int)hipTransformer2.ySize,
				 false,
				 1);
	CTOC(timer,"PRE-CenterFFT_0");

	hipTransformer2.reals.cpOnAcc(d_Mmean); //TODO remove the need for this

	CTIC(timer,"PRE-multi_2");
	Bsize = ( (int) ceilf(( float)d_Fmsk.getSize()/(float)BLOCK_SIZE));
	hipLaunchKernel(hip_kernel_convol_A,Bsize,BLOCK_SIZE, 	  ~d_Fmsk,
													  ~d_Fmic2,
													  ~d_Fcov,
													  d_Fmsk.getSize());
	LAUNCH_HANDLE_ERROR(hipGetLastError());
	...
	Bsize = ( (int) ceilf(( float)d_Mstddev.getSize()/(float)BLOCK_SIZE));
		hipLaunchKernel(hip_kernel_finalizeMstddev,Bsize,BLOCK_SIZE,
		~d_Mstddev,
		~hipTransformer2.reals,
		normfft,
		d_Mstddev.getSize());
...
void AutoPickerHip::autoPickOneMicrograph(FileName &fn_mic, long int imic)
{
	Image<RFLOAT> Imic;
	...
	if(!basePckr->do_read_fom_maps)
	{
		...
		CTIC(timer,"setSize_hipTr");
		hipTransformer1.setSize(basePckr->workSize,basePckr->workSize, 1, Npsi, FFTW_BACKWARD);
		CTOC(timer,"setSize_hipTr");
	}
	...
	if(hipTransformer1.batchSize.size()>1 && !have_warned_batching)
	{
		...
		std::cerr 			   << "since one or more micrographs has to use (at least " << hipTransformer1.batchSize.size() << ") batches of orientations to "<< std::endl;
	...
	else
	{
		...
		hipLaunchKernel(HipKernels::hip_kernel_multi<XFLOAT>,FMultiBsize,BLOCK_SIZE,
		...
		hipLaunchKernel(hip_kernel_square,FMultiBsize,BLOCK_SIZE,
	...
	if(basePckr->do_ctf)
	{
		hipLaunchKernel(hip_kernel_rotateAndCtf,blocks,BLOCK_SIZE,
	...
	{
		hipLaunchKernel(hip_kernel_rotateOnly,blocks,BLOCK_SIZE,
													  ~hipTransformer1.fouriers,
													  0,
													  projKernel,
													  0
	...
	basePckr->timer.tic(basePckr->TIMING_B5);
			...
			CTIC(timer,"AllPsi");
			int startPsi(0);
			for (int psiIter = 0; psiIter < hipTransformer1.batchIters; psiIter++) // psi-batches for possible memory-limits
			{
				...
				hipLaunchKernel(hip_kernel_batch_convol_A,blocks2,BLOCK_SIZE,
					~hipTransformer1.fouriers,
					~d_Fmic,
					FauxStride);
				LAUNCH_HANDLE_ERROR(hipGetLastError());
				...
				CTIC(timer,"HipInverseFourierTransform_1");
				hipTransformer1.backward();
				HANDLE_ERROR(hipDeviceSynchronize());
				CTOC(timer,"HipInverseFourierTransform_1");


				CTIC(timer,"runCenterFFT_1");
				runCenterFFT(hipTransformer1.reals,
							(int)hipTransformer1.xSize,
							 (int)hipTransformer1.ySize,
							 false,
							 hipTransformer1.batchSize[psiIter]);
				...
				hipLaunchKernel(hip_kernel_probRatio,PR_blocks,PROBRATIO_BLOCK_SIZE,
						~d_Mccf_best,
						~d_Mpsi_best,
						~hipTransformer1.reals,
						~d_Mmean,
						~d_Mstddev,
						hipTransformer1.reals.getSize()/hipTransformer1.batchSize[0],
						(XFLOAT) -2*normfft,
				...
				LAUNCH_HANDLE_ERROR(hipGetLastError());
				startPsi += hipTransformer1.batchSize[psiIter];
