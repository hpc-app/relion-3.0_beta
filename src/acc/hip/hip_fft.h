...
//		printf("%15li\n", avail);
	    ...
	    if(direction<=0)
	    {
	    	HANDLE_HIPFFT_ERROR( hipfftPlanMany(&hipfftPlanForward,  dimension, inembed, inembed, istride, idist, onembed, ostride, odist, HIPFFT_D2Z,\
 batchSize[0]));
	   		HANDLE_HIPFFT_ERROR( hipfftSetStream(hipfftPlanForward, fouriers.getStream()));
	    }
	    if(direction>=0)
	    {
	    	HANDLE_HIPFFT_ERROR( hipfftPlanMany(&hipfftPlanBackward, dimension, inembed, onembed, ostride, odist, inembed, istride, idist, HIPFFT_Z2D,\
 batchSize[0]));
	...
	void backward(AccPtr<hipfftDoubleReal> &dst)
		{ HANDLE_HIPFFT_ERROR( hipfftExecZ2D(hipfftPlanBackward, ~fouriers, ~dst) ); }
	 	...
	 	if(direction<=0)
	 	{
	 		HANDLE_HIPFFT_ERROR( hipfftPlanMany(&hipfftPlanForward,  dimension, inembed, inembed, istride, idist, onembed, ostride, odist,\
 HIPFFT_R2C, batchSize[0]));
	 		HANDLE_HIPFFT_ERROR( hipfftSetStream(hipfftPlanForward, fouriers.getStream()));
	 	}
	 	if(direction>=0)
	 	{
	 		HANDLE_HIPFFT_ERROR( hipfftPlanMany(&hipfftPlanBackward, dimension, inembed, onembed, ostride, odist, inembed, istride, idist,\
 HIPFFT_C2R, batchSize[0]));
	 		HANDLE_HIPFFT_ERROR( hipfftSetStream(hipfftPlanBackward, reals.getStream()));
	 	}
		planSet = true;
	}
